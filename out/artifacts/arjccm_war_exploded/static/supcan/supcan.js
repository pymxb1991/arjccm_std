 ...
 * 实例化一个报表控件（已废弃，可删除）
 * @param AF 报表对象
 * @param url 报表描述文件URL，如果不是一个URL，则填写实体类名
 * @param options 参数选项
 * @returns {Supcan} 报表封装对象实例
 */
...
/**
 * 报表控件加载，id参数说明：如果页面中有多个插件/加载项，那么OnReady事件会被触发多次，id就是插件/加载项的id
 */
function OnReady(id){
...
/**
 * 报表控件事件，参数说明: “Event”为事件名，p1,p2,p3,p4参数有特定的含义，由具体的功能组件规定.
 */
...
function Supcan(AF, url, options){
	...
	/**
	 * 报表数据对象，如果dataType为json，则为JSON对象，否则为数据字符串
	 */
	$this.data = "";
