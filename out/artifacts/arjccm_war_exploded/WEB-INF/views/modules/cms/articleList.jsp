...
<body>
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/" method="post" class="breadcrumb form-search">
		...
		<input id="category" name="category.id" type="hidden" value="${article.category.id}"/>
		<label>栏目：</label>${article.category.name}&nbsp;&nbsp;&nbsp;&nbsp;<%-- <sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name" labelValue="${article.category.name}"
