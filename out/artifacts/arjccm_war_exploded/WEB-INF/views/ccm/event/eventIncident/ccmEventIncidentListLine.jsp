...
<%--<span class="nav-position">当前位置 ：</span><span class="nav-menu"><%=session.getAttribute("activeMenuName")%>></span><span class="nav-menu2">护路护线</span>--%>
<ul class="back-list">
<div class="context" content="${ctx}"></div>
	<ul class="nav nav-tabs">
		<li class="active"><a class="nav-head" href="${ctx}/event/ccmEventIncident/listLine">线路案（事）件列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmEventIncident"
		action="${ctx}/event/ccmEventIncident/listLine" method="post"
		class="breadcrumb form-search clearfix">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden"
			value="${page.pageSize}" />
		<ul class="ul-form pull-left">
			<li class="first-line"><label>案（事）件名称：</label> <form:input path="caseName"
			...
			<li class="first-line"><label >结束日期：</label><input
					name="endHappenDate" type="text" readonly="readonly"
					maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${ccmEventIncident.endHappenDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false}); " style="margin-left: 6px" />
			</li>
			...
			<li class="first-line"><label>所属线、路：</label>
				<sys:treeselect id="line" name="line.id" value="${ccmEventIncident.line.id}" labelName="line.name" labelValue="${ccmEventIncident.line.name}"
					title="线、路名称" url="/event/ccmEventIncident/treeData" cssClass="" allowClear="true" notAllowSelectParent="true" cssStyle="width: 158px"/>
			</li>

			<li class="second-line"><label>案（事）件分级：</label> <form:select path="eventScale"
					class="input-medium">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmEventIncident">
				<tr>
					...
					<td><a href="${ctx}/event/ccmEventIncident/formLine?id=${ccmEventIncident.id}">
							${ccmEventIncident.caseName} </a></td>
					<td>${ccmEventIncident.number}</td>
					<td>${ccmEventIncident.line.name}</td>
					<td><fmt:formatDate value="${ccmEventIncident.happenDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<%-- <td>${ccmEventIncident.casePlace}</td> --%>
					<td>
					<span class="eventScaleCss eventScaleCss-${ccmEventIncident.eventScale}">${fns:getDictLabel(ccmEventIncident.eventScale, 'ccm_case_grad', '')}
					</span></td>
...
<%--					<td>${fns:getDictLabel(ccmEventIncident.property, 'ccm_case_natu', '')}</td>--%>
					...
					<td>
						<!-- 案（事）件登记编辑权限  --> <shiro:hasPermission
							...
							<a  class="btnList"
								href="${ctx}/event/ccmEventIncident/formLine?id=${ccmEventIncident.id}" title="修改"><i class="icon-pencil"></i></a>
							<a	class="btnList"
								href="${ctx}/event/ccmEventIncident/deleteLine?id=${ccmEventIncident.id}"
								onclick="return confirmx('确认要删除该案（事）件吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
							...
							<!-- 
							<a class="btnList" href="${ctx}/event/ccmEventCasedeal/dealformLine?eventIncidentId=${ccmEventIncident.id}" title="添加处理信息"><i class="icon-plus"></i></a>
							 -->
