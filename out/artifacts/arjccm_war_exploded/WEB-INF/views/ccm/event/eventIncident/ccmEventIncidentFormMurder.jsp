...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="event:ccmEventCasedeal:edit">
			<li><a
				href="${ctx}/event/ccmEventCasedeal/dealformSDMurder?eventIncidentId=${ccmEventIncident.id}">事件处理添加</a></li>
		</shiro:hasPermission>
		  -->
		<!--  只读处理权限  
	...
	<form:form id="inputForm" modelAttribute="ccmEventIncident"
		action="${ctx}/event/ccmEventIncident/saveMurder" method="post"
		...
		<table border="0px" style="border-color: #CCCCCC; border: 0px solid #CCCCCC; padding: 10px; width: 100%;">
				...
				<td>
					<div>
						...
						<div class="controls">
							...
							notAllowSelectParent="false" cssStyle="width: 150px" cssClass=""/>
			...
			<tr>
				<td>
					<div>
						<label class="control-label"><span class="help-inline"><font color="red">*</font></span> 案件名称：</label>
						<div class="controls">
							...
							<tr>
								...
								<shiro:hasPermission name="event:ccmEventCasedeal:edit">
									<li style="list-style-type: none;"><a
										href="${ctx}/event/ccmEventCasedeal/dealformSDMurder?id=${ccmeventcasedeal.id}">
											<fmt:formatDate value="${ccmeventcasedeal.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />&nbsp; 
											指派&nbsp; ${ccmeventcasedeal.handleUser.name}&nbsp; 
											处理此案（事）件&nbsp;${fns:getDictLabel(ccmeventcasedeal.handleStatus, 'ccm_event_status', '')}
									</a></li>
								</shiro:hasPermission>
								   -->
								<!-- 只读处理权限   -->
								<shiro:hasPermission name="event:ccmEventCasedeal:viewRead">
									...
									<a onclick="parent.LayerDialog('${ctx}/event/ccmEventCasedeal/detail?id=${ccmeventcasedeal.id}', '处理详情', '1000px', '700px')">
											<fmt:formatDate value="${ccmeventcasedeal.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />&nbsp; 
											指派&nbsp; ${ccmeventcasedeal.handleUser.name}&nbsp; 
											处理此案（事）件&nbsp;${fns:getDictLabel(ccmeventcasedeal.handleStatus, 'ccm_event_status', '')}
									</a></li>
