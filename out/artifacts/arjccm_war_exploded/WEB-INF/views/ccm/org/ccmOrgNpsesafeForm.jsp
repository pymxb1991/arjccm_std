...
<body>
	<ul class="nav nav-tabs">
		<%-- <li><a href="${ctx}/org/ccmOrgNpsesafe/">安全生产重点列表</a></li> 
		<li class="active"><a href="${ctx}/org/ccmOrgNpsesafe/form?id=${ccmOrgNpse.id}">安全生产重点<shiro:hasPermission name="org:ccmOrgNpsesafe:edit">${not empty ccmOrgNpse.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="org:ccmOrgNpsesafe:edit">查看</shiro:lacksPermission></a></li>--%>
	<%-- 	<c:if test="${not empty ccmOrgNpse.id}">
			<li><a
				href="${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npsesafe">跟踪信息<shiro:hasPermission
