...
<ul class="back-list">
	...
	<form:form id="searchForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpseElse/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>安全隐患类型：</label>
...
<%--			<li class="clearfix"></li>--%>
	...
	<div class="clearfix pull-right btn-box">
		<a
				onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form', '添加', '1100px', '600px')"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		<thead>
			<tr>
				...
				<th>负责人姓名</th>
				<th>负责人联系方式</th>
				<shiro:hasPermission name="org:ccmOrgNpseElse:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="ccmOrgNpse">
			<tr>
				...
				<td>
					<a onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form?id=${ccmOrgNpse.id}', '编辑', '1100px', '800px')">${ccmOrgNpse.compName}</a>
<%--					<a href="${ctx}/org/ccmOrgNpseElse/form?id=${ccmOrgNpse.id}">--%>
<%--					${ccmOrgNpse.compName}</a>--%>
				</td>
				<td>${fns:getDictLabel(ccmOrgNpse.compType, 'ccm_buss_cate', '')}</td>
				<td>
				...
				<td><shiro:hasPermission name="org:ccmOrgNpseElse:edit">
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form?id=${ccmOrgNpse.id}', '编辑', '1100px', '600px')"title="修改"><i class="icon-pencil"></i></a>
					<a class="btnList" href="${ctx}/org/ccmOrgNpseElse/delete?id=${ccmOrgNpse.id}" onclick="return confirmx('确认要删除该其他重点吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
					</shiro:hasPermission> <shiro:hasPermission name="log:ccmLogTail:edit">
<%--					<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseElse', '添加记录', '800px', '660px')"--%>
