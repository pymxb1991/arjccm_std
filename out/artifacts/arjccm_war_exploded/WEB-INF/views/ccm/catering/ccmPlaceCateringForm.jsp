...
<body>
	<ul class="nav nav-tabs">
		<%-- 		<li class="active"><a
			href="${ctx}/catering/ccmPlaceCatering/form?id=${ccmPlaceCatering.id}">餐饮场所<shiro:hasPermission
					name="catering:ccmPlaceCatering:edit">${not empty ccmPlaceCatering.id?'修改':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="catering:ccmPlaceCatering:edit">查看</shiro:lacksPermission></a></li> --%>
	</ul>
	<form:form id="inputForm" modelAttribute="ccmPlaceCatering"
		action="${ctx}/catering/ccmPlaceCatering/save" method="post"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceCatering.ccmBasePlace.administrativeDivision}" labelName=""
							labelValue="${ccmPlaceCatering.ccmBasePlace.area.name}"
				...
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font></span>成立时间：</label>
					<div class="controls">
						<input name="ccmBasePlace.createTime" type="text"
							...
							value="<fmt:formatDate value="${ccmPlaceCatering.ccmBasePlace.createTime}" pattern="yyyy-MM-dd"/>"
		...
		<h4 class="tableName">餐馆信息</h4>
		<table>
			<tr>
				<td><label class="control-label">营业时间：</label>
					<div class="controls">
						<form:input path="businessHours" htmlEscape="false"
							maxlength="255" class="input-xxlarge " />
					</div></td>
			</tr>
			<tr>
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font></span>餐饮范围：</label>
					<div class="controls">
						<form:textarea path="cateringScope" htmlEscape="false" maxlength="255"
							rows="2" class="input-xxlarge required" />
					</div></td>
			</tr>
			<tr>
