...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/traffic/ccmPlaceTraffic/03">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmPlaceTraffic"
		action="${ctx}/traffic/ccmPlaceTraffic/03" method="post"
...
<%--			<li class="clearfix"></li>--%>
	...
	<div class="clearfix pull-right btn-box">
		<a
				onclick="parent.LayerDialog('${ctx}/traffic/ccmPlaceTraffic/form?type=03', '添加', '1100px', '700px')"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceTraffic">
				<tr>
					...
					<td>${fns:getDictLabel(ccmPlaceTraffic.gasStationType, 'ccm_station_type1', '无')}</td>
					<td><fmt:formatDate value="${ccmPlaceTraffic.updateDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<shiro:hasPermission name="traffic:ccmPlaceTraffic:edit">
						<td>
						<a class="btnList"
							onclick="parent.LayerDialog('${ctx}/event/ccmEventIncident/listCheck?placeId=${ccmPlaceTraffic.basePlaceId}', '事件关联', '1100px', '700px')"
