...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#"><shiro:hasPermission name="oa:ccmOfficialDocument:edit">${ccmOfficialDocument.act.taskName}</shiro:hasPermission><shiro:lacksPermission name="oa:ccmOfficialDocument:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<br/>
	<form:form id="inputForm" modelAttribute="ccmOfficialDocument" action="${ctx}/oa/ccmOfficialDocument/saveAudit" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="drafterTel" htmlEscape="false" maxlength="30" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="oa:ccmOfficialDocument:edit">
				<c:if test="${ccmOfficialDocument.act.taskDefKey eq 'app2'}">
					<input id="btnSubmit" class="btn btn-primary" type="submit" value="兑 现" onclick="$('#flag').val('yes')"/>&nbsp;
				</c:if>
				<c:if test="${ccmOfficialDocument.act.taskDefKey ne 'app2'}">
