...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/pop/ccmPermanent/form?id=${ccmPeople.id}">数据<shiro:hasPermission name="pop:ccmPermanent:edit">${not empty ccmPeople.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="pop:ccmPermanent:edit">查看</shiro:lacksPermission></a></li>
	...
	<form:form id="inputForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmPermanent/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<ul class="nav nav-pills ulz" style="display:none;">
					<li><h4 id="zd1">
							<i class="icon-tags"></i>治安重点人员标记：
						</h4>
					<li><c:if test="${ccmPeople.isDrugs==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/specialform?id=${ccmPeople.id}', '吸食毒品人员标记', '1200px', '900px')">吸食毒品人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isVisit==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/house/ccmHousePetition/specialform?id=${ccmPeople.id}', '非正常上访人员标记', '1200px', '900px')">非正常上访人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isPsychogeny==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/house/ccmHousePsychogeny/specialform?id=${ccmPeople.id}', '易肇事肇祸精神病人标记', '1200px', '900px')">肇事肇祸精神病人标记</a></li>
						</c:if>
						<c:if test="${ccmPeople.isCriminalOffense==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/seriousCriminalOffenseform?id=${ccmPeople.id}', '严重刑事犯罪活动嫌疑人员标记', '1200px', '900px')">严重刑事犯罪活动嫌疑人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isHarmNational==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/specialHarmNationalform?id=${ccmPeople.id}', '危害国家安全活动嫌疑人员标记', '1200px', '900px')">危害国家安全活动嫌疑人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isEscape==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/escapePeopleform?id=${ccmPeople.id}', '在逃人员标记', '1200px', '900px')">在逃人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isDispute==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/specialHouseDisputeform?id=${ccmPeople.id}', '闹事行凶报复嫌疑人员标记', '1200px', '900px')">闹事行凶报复嫌疑人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isDeliberatelyIllegal==1}">
							<li><a class="znul"
								onclick="parent.LayerDialog('${ctx}/house/ccmHouseDeliberatelyIllegal/specialform?id=${ccmPeople.id}', '故意违法刑释不足5年人员标记', '1200px', '900px')">故意违法刑释不足5年人员标记</a></li>
						</c:if> <c:if
						...
						<c:if
							...
							test="${ccmPeople.isEscape==0||ccmPeople.isEscape==null||ccmPeople.isEscape==''}">
							<li><a
								onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/escapePeopleform?id=${ccmPeople.id}', '在逃人员标记', '1200px', '900px')">在逃人员标记</a></li>
						</c:if> <c:if
				...
				<ul class="nav nav-pills ulz" style="display:none;">
					<li><h4 id="zd2">
							<i class="icon-tags"></i>综治重点人员标记：
						</h4>
					<li><c:if test="${ccmPeople.isRelease==1}">
							<li><a class="zzul"
								onclick="parent.LayerDialog('${ctx}/house/ccmHouseRelease/specialform?id=${ccmPeople.id}', '刑释教解人员标记', '1200px', '900px')">刑释教解人员标记</a></li>
				...
				<ul class="nav nav-pills ulz" style="display:none;">
					<li><h4 id="zd3">
							<i class="icon-tags"></i>帮扶关注人员标记：
						</h4>
					<li><c:if test="${ccmPeople.isBehind==1}">
							<li><a class="gzul"
								onclick="parent.LayerDialog('${ctx}/pop/ccmPopBehind/specialform?id=${ccmPeople.id}', '留守人员标记', '1200px', '900px')">留守人员标记</a></li>
						</c:if> <c:if test="${ccmPeople.isKym==1}">
							<li><a class="gzul"
								onclick="parent.LayerDialog('${ctx}/house/ccmHouseKym/specialform?id=${ccmPeople.id}', '重点青少年标记', '1200px', '900px')">重点青少年标记</a></li>
						</c:if> <c:if
							test="${ccmPeople.isBehind==0||ccmPeople.isBehind==null||ccmPeople.isBehind==''}">
							<li><a
								onclick="parent.LayerDialog('${ctx}/pop/ccmPopBehind/specialform?id=${ccmPeople.id}', '留守人员标记', '1200px', '900px')">留守人员标记</a></li>
						</c:if> <c:if
