...
<script type="text/javascript"
	src="${ctxStatic}/ccm/pop/js/ccmPopBehindInfo.js"></script>
	<script type="text/javascript"
...
<div class="context" content="${ctx}"></div>
	...
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/pop/ccmPopBehind/import"
	...
	<form:form id="searchForm" modelAttribute="ccmPopBehind"
		...
		<ul class="ul-form pull-left">
			<li class="first-line"><label>姓名：</label> <form:input path="name"
					htmlEscape="false" maxlength="50" class="input-medium" /></li>
			<li class="first-line"><label>性别：</label> <form:select path="sex"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('sex')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="first-line"><label>关注程度：</label> <form:select path="atteType"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_conc_exte')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="first-line"><label>健康状况：</label> <form:select path="health"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_phy_cdt')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="first-line"><label>留守人员类型：</label> <form:select path="staytype"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_rear_type')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="second-line"><label>家庭主要成员姓名：</label> <form:input path="crucialname"
					htmlEscape="false" maxlength="50" class="input-medium" /></li>
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPopBehind">
				<tr>
					<td width="100px">
						<img src="${ccmPopBehind.images}" style="height:50px;" class="pimg"/>
					</td>
					<td class="tc">
					<a class="btnList"
									onclick="parent.LayerDialog('${ctx}/pop/ccmPopBehind/form?id=${ccmPopBehind.id}', '编辑', '1200px', '900px')"
							title="修改"> ${ccmPopBehind.name}</a>
					<td class="tc">${ccmPopBehind.ident}</td>
					<td class="tc">${fns:getDictLabel(ccmPopBehind.sex, 'sex', '')}</td>
					<td class="tc">${ccmPopBehind.telephone}</td>
					<td class="tc">${fns:getDictLabel(ccmPopBehind.health, 'ccm_phy_cdt', '')}</td>
					<c:if test="${ccmPopBehind.atteType eq '01'}">
						<td style='color: red'>${fns:getDictLabel(ccmPopBehind.atteType, 'ccm_conc_exte', '')}&nbsp;
							&nbsp; <img src="${ctxStatic}/images/atteType_red.png" />
						</td>
					</c:if>
					<c:if test="${ccmPopBehind.atteType eq '02'}">
						<td style='color: orange'>${fns:getDictLabel(ccmPopBehind.atteType, 'ccm_conc_exte', '')}&nbsp;
							&nbsp; <img src="${ctxStatic}/images/atteType_orange.png" />
						</td>
					</c:if>
					<c:if test="${ccmPopBehind.atteType eq '03'}">
					...
					<td class="tc">${fns:getDictLabel(ccmPopBehind.staytype, 'ccm_rear_type', '')}
					</td>
					<td class="tc">${ccmPopBehind.crucialname}</td>
					<td class="tc"><shiro:hasPermission name="pop:ccmPopBehind:edit">
							<a class="btnList"
									onclick="parent.LayerDialog('${ctx}/pop/ccmPopBehind/form?id=${ccmPopBehind.id}', '编辑', '1200px', '900px')"
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmPopBehind.id}&relevance_table=ccm_pop_behind', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmPopBehind.id}&relevance_table=ccm_pop_behind', '添加记录', '800px', '660px')"
