						...
						$('#ccmBasePlaceCommon')
								.attr(
										...
										'${ctx}/place/ccmBasePlace/form/common?id=${ccmPlaceLive.basePlaceId}&name='
												+ encodeURI('农贸市场信息')
												+ '&url=${ctx}/live/ccmPlaceLive/form?type=3&id=${ccmPlaceLive.id}')
...
<body>
	...
	<form:form id="inputForm" modelAttribute="ccmPlaceLive"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceLive.ccmBasePlace.administrativeDivision}"
							labelName="" labelValue="${ccmPlaceLive.ccmBasePlace.area.name}"
							title="网格" url="/tree/ccmTree/treeDataArea?type=7&areaid=" allowClear="true" cssClass=""
		...
		<h4 class="tableName">农贸市场信息</h4>
		<table>
			<tr>
				<td style="width: 50%;"><label class="control-label"><span class="help-inline"><font color="red">*</font></span>证照情况：</label>
					<div class="controls">
						<form:checkboxes path="certificationList"
							items="${fns:getDictList('ccm_certificate_type')}"
							itemLabel="label" itemValue="value" htmlEscape="false"
							class="required" />
					</div></td>
			</tr>
			<tr>
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font></span>经营范围：</label>
					<div class="controls">
						<form:textarea path="businessScope" htmlEscape="false" rows="2"
