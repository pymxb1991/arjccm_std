...
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHousePsychogeny/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
			<input id="uploadFile" name="file" type="file" style="width: 330px" /><br /> <br />
			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="导  入 " />
		</form>
	</div>
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatPsychogeny">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHousePsychogeny" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHousePsychogeny" action="${ctx}/house/ccmHousePsychogeny/" method="post" class="breadcrumb form-search clearfix">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHousePsychogeny">
				<tr>
					...
					<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHousePsychogeny/form?id=${ccmHousePsychogeny.id}', '信息', '1200px', '900px')">${ccmHousePsychogeny.name}</a></td>
					...
					<td class="tc"><shiro:hasPermission name="house:ccmHousePsychogeny:edit">
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHousePsychogeny/form?id=${ccmHousePsychogeny.id}', '修改', '1200px', '900px')" title="修改"><i class="icon-pencil"></i></a>
						<a class="btnList" href="${ctx}/house/ccmHousePsychogeny/delete?id=${ccmHousePsychogeny.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该肇事肇祸等严重精神障碍患者吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
						<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHousePsychogeny.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHousePsychogeny.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="icon-group"></i></a>
						<%-- <a class="btnList" onclick="parent.LayerDialog1('','${ctx}/work/ccmWorkTiming/form', '定时提醒', '700px', '500px')" title="定时提醒"><i class="icon-bell"></i></a> --%>
			  			...
			  			<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHousePsychogeny.id}&relevance_table=ccm_house_psychogeny', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
			  			<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHousePsychogeny.id}&relevance_table=ccm_house_psychogeny', '添加记录', '800px', '660px')"  title="添加记录"><i class="icon-plus"></i></a>
