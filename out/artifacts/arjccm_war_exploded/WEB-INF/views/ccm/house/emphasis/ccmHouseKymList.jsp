...
<div class="context" content="${ctx}"></div>
	<form:form id="searchForm" modelAttribute="ccmHouseKym" action="${ctx}/house/ccmHouseKym/?tableType='ccmHouseKym'" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>家庭情况：</label> <form:select path="famiStat" class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_famy_stat')}" itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select>
			</li>
			<li class="first-line"><label >是否违法犯罪：</label>
	...
	<div class="clearfix pull-right btn-box">
			...
			<a href="javascript:;" onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseEmphasis/listSet?type=ccmHouseKym', '通知信息设置', '550px', '250px')" id="btnSetting" style="width: 70px!important;display:inline-block;float: right;margin-left: 20px;" class="btn btn-export">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseKym">
				<tr>
					...
					<td><a href="${ctx}/house/ccmHouseKym/form?id=${ccmHouseKym.id}">${fns:getDictLabel(ccmHouseKym.manType, 'ccm_delp_type', '')}</a></td>
					...
					<td><shiro:hasPermission name="house:ccmHouseKym:edit">
						<a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseKym/form?id=${ccmHouseKym.id}&hide1=true&hide2=false', '详情', '1200px', '800px')" title="详情"><i class="icon-pencil"></i></a>
						<a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseKym/form?id=${ccmHouseKym.id}&hide1=false&hide2=true', '走访记录', '1200px', '500px')" title="走访记录"><i class="icon-list-ul"></i></a>
							...
							<%-- <a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseKym.id}&relevance_table=ccm_house_kym', '记录信息', '800px', '660px')" title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a> --%>
