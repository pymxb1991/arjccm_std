...
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHarmNationalSecurity/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
			<input id="uploadFile" name="file" type="file" style="width: 330px" /><br /> <br />
			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="导  入 " />
		</form>
	</div>
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatHarmNational">数据统计</a></li></shiro:hasPermission>
		<li class="active"  style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHarmNationalSecurity" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHarmNationalSecurity" action="${ctx}/house/ccmHarmNationalSecurity/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label >危险程度：</label> <form:select path="dangerLevel" class="input-medium">
				<form:option value="" label="全部" />
				<form:options items="${fns:getDictList('danger_level_dict')}" temLabel="label" itemValue="value" htmlEscape="false" />
			</form:select></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<c:forEach items="${page.list}" var="ccmHarmNationalSecurity">
			<tr>
				...
				<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHarmNationalSecurity/form?id=${ccmHarmNationalSecurity.id}', '信息', '1200px', '900px')">${ccmHarmNationalSecurity.name}</a></td>
				...
				<c:if test="${ccmHarmNationalSecurity.dangerLevel eq '01'}">
					<td style='color:red'>${fns:getDictLabel(ccmHarmNationalSecurity.dangerLevel, 'danger_level_dict', '')}&nbsp; &nbsp; <img src="${ctxStatic}/images/atteType_red.png" /> </td>
				</c:if>
				<c:if test="${ccmHarmNationalSecurity.dangerLevel eq '02'}">
					<td style='color:orange'>${fns:getDictLabel(ccmHarmNationalSecurity.dangerLevel, 'danger_level_dict', '')}&nbsp; &nbsp; <img src="${ctxStatic}/images/atteType_orange.png" /> </td>
				</c:if>
				<c:if test="${ccmHarmNationalSecurity.dangerLevel eq '03'}">
					<td>${fns:getDictLabel(ccmHarmNationalSecurity.dangerLevel, 'danger_level_dict', '')}&nbsp; &nbsp; <img src="${ctxStatic}/images/atteType_green.png" /> </td>
				</c:if>
				...
				<td class="tc"><shiro:hasPermission name="house:ccmHarmNationalSecurity:edit">
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHarmNationalSecurity/form?id=${ccmHarmNationalSecurity.id}', '修改', '1200px', '900px')" title="修改"><i class="icon-pencil"></i></a>
					<a class="btnList" href="${ctx}/house/ccmHarmNationalSecurity/delete?id=${ccmHarmNationalSecurity.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该危害国家安全活动嫌疑人口吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHarmNationalSecurity.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHarmNationalSecurity.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="icon-group"></i></a>
					...
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHarmNationalSecurity.id}&relevance_table=ccm_harm_national_security', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHarmNationalSecurity.id}&relevance_table=ccm_harm_national_security', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>
