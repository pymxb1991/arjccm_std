...
<div class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/hospital/ccmPlaceHospital/03">数据列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="ccmPlaceHospital"
		action="${ctx}/hospital/ccmPlaceHospital/03" method="post"
...
<%--			<li class="clearfix"></li>--%>
	...
	<div class="clearfix pull-right btn-box">
		<a
				onclick="parent.LayerDialog('${ctx}/hospital/ccmPlaceHospital/form?type=03', '添加', '1100px', '700px')" style="width: 49px;display:inline-block;float: right;"
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceHospital">
				<tr>
					<td width="200px"><img
						src="${ccmPlaceHospital.ccmBasePlace.placePicture}"
						style="height: 50px;" class="pimg"/></td>
					<td><a class="btnList"
						onclick="parent.LayerDialog('${ctx}/hospital/ccmPlaceHospital/form?id=${ccmPlaceHospital.id}', '编辑', '1100px', '700px')">${ccmPlaceHospital.placeName}</a>
					</td>
					<td>${ccmPlaceHospital.leaderName}</td>
					<td>${ccmPlaceHospital.leaderContact}</td>
					<td>${ccmPlaceHospital.pharmacistNumber}</td>
					<td><a
					...
					<shiro:hasPermission name="hospital:ccmPlaceHospital:edit">
						<td>
						<a class="btnList"
							onclick="parent.LayerDialog('${ctx}/event/ccmEventIncident/listCheck?placeId=${ccmPlaceHospital.basePlaceId}', '事件关联', '1100px', '700px')"
							title="事件关联"><i class="icon-random"></i></a> 
						<a class="btnList"
							onclick="parent.LayerDialog('${ctx}/hospital/ccmPlaceHospital/form?id=${ccmPlaceHospital.id}', '编辑', '1100px', '700px')"
