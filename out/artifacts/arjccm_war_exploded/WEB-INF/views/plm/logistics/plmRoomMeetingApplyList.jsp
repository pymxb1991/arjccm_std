...
<body>
	<form:form id="searchForm" modelAttribute="plmRoomApply" action="${ctx}/logistics/plmRoomMeetingApply/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="plmRoomApply">
			<tr>
				<td><a href="${ctx}/logistics/plmRoomMeetingApply/form?id=${plmRoomApply.id}">
					${plmRoomApply.subject}
				</a></td>
				<td>
					${fns:getDictLabel(plmRoomApply.isEnd, 'yes_no', '')}
				</td>					
				<td>
					${fns:getDictLabel(plmRoomApply.type, 'plm_room_apply_type', '')}
				</td>
				<td>
				...
				<td>
					<c:if test="${not empty plmRoomApply.procInsId}" var="condition">
								<a
										href="${ctx}/logistics/plmRoomMeetingApply/form?id=${plmRoomApply.id}"
					...
					<c:if test="${!condition}">
							...
							<a class="btnList"
								href="${ctx}/logistics/plmRoomMeetingApply/delete?id=${plmRoomApply.id}"
			...
			<c:forEach items="${page.list}" var="plmRoomApply">
				<div class="span4 spandiv">
					<div class="thumbnail">
						...
						<a href="${ctx}/logistics/plmRoomMeetingApply/form?id=${plmRoomApply.id}">
							<h4 title="${plmRoomApply.code}">申请编号：${plmRoomApply.code}</h4>
						</a>
						<div class="caption row-fluid">

							<div class="spantext  " style="width: 84%; margin-left: 7%">
								<p title="${plmRoomApply.subject}">会议名称：${plmRoomApply.subject}</p>			
							   
								<p title="${fns:getDictLabel(plmRoomApply.type, 'plm_room_apply_type', '')}">会议类型：${fns:getDictLabel(plmRoomApply.type, 'plm_room_apply_type', '')}</p>
								<p title="${plmRoomApply.room.subject}">会议地点：${plmRoomApply.room.subject}</p>																								
							</div>
						</div>
