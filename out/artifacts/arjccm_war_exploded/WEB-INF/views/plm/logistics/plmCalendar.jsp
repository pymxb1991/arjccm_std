...
$(document).ready(function() {
	...
	for (var i = 0; i < keyList.length; i++) {
		keyList[i][2]=new Date(keyList[i][2]);
		keyList[i][3]=new Date(keyList[i][3]);
	}
 ...
 function Edit(data)
{
   var eurl="${ctx}/calendar/plmCalendar/formdate?data="+data;   
	...
	{     
		...
		alert(url)
		OpenModelWindow(url,{ width: 1000, height: 560, caption:"管理日程",onclose:function(){
...
{   
	...
	{   
		...
		OpenModelWindow(url,{ width: 1000, height:560, caption: "编辑日程"});
	}                
}    
 function Delete(data,callback)
{  alert("111" +data+callback)
	$.alerts.okButton="确定";  
...
$("#faddbtn").click(function(e) {
	var url ="${ctx}/calendar/plmCalendar/form";
	OpenModelWindow(url,{ width:1000, height: 560, caption: "新增日程"});
});
//点击回到今天
