...
public class PlmEquApplyService extends CrudService<PlmEquApplyDao, PlmEquApply> {
	...
	public void notCommit(PlmEquApply plmEquApply) {
		...
		plmActService.save(plmEquApply.getPlmAct(), PlmTypes.EQU_APPLY_RECEIVE, plmEquApply.getId());
	}
	...
	public void save(PlmEquApply plmEquApply) {
		...
		if (StringUtils.isBlank(plmEquApply.getProcInsId())) {
			...
			Map<String, String> returnMap = actUtConfigService.getProcInsId(PlmTypes.EQU_APPLY_RECEIVE, plmEquApply,
					plmEquApply.getId());
	...
	//统计本年度装备申请次数（领用申请，报废申请，维修保养申请）
	public List<EchartType> selectNumEquApply(){
		return dao.selectNumEquApply();
	}
