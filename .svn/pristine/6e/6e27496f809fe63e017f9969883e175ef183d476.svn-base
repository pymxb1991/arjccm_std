		...
		$(document).ready(function() {
			...
			$("#btnCancel").on("click", function(){
				...
				/* confirmx("确定要撤销申请吗？",function(){ */
					$('#flag').val('no');
					$("#inputForm").attr("action","${ctx}/travel/plmApplicationForLeave/apply");
					$("#inputForm").submit();
				/* }); */
			});
			$("#btnSubmit").on("click", function(){
				$("#inputForm").attr("action","${ctx}/travel/plmApplicationForLeave/save");
				$("#inputForm").submit();
			});
...
<body>
	<form:form id="inputForm" modelAttribute="plmApplicationForLeave" action="${ctx}/travel/plmApplicationForLeave/save" method="post" class="form-horizontal">
		...
		<table id="tabletop" class="table">
			<tr>
				<td class="tabletop" colspan="2" width="33.33%">申请人<span class="help-inline"><font color="red">*</font> </span>:&nbsp;&nbsp; &nbsp;&nbsp;<span>${plmApplicationForLeave.applyer.name}</span>&nbsp;&nbsp;&nbsp;&nbsp;</td>
				<td class="tabletop" colspan="2" width="33.33%">所属部门<span class="help-inline"><font color="red">*</font> </span>: &nbsp;&nbsp; &nbsp;&nbsp;<span>${plmApplicationForLeave.department.name}</span>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		...
		<table id="table" class="table table-condensed">
			...
			<tr>
				...
				<td colspan="2"><input name="fromDate" id="fromDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${plmApplicationForLeave.fromDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
				...
				<td colspan="2"><input name="toDate" id="toDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${plmApplicationForLeave.toDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/></td>
			</tr>
			<tr>
				<td class="trtop" colspan="1">请假类型<span class="help-inline"><font color="red">*</font> </span></td>
				...
				<td class="trtop" colspan="1">请假时长(天)<span class="help-inline"><font color="red">*</font> </span></td>
				<td colspan="2"><form:input path="days" htmlEscape="false" class="input-xlarge required number"/></td>
			</tr>
			<tr>
				<td class="trtop" colspan="1">请假原因<span class="help-inline"><font color="red">*</font> </span></td>
			...
			<tr>
				...
				<td colspan="5">
					...
					<sys:ckfinder input="file" type="files" uploadPath="/travel/plmApplicationForLeave" selectMultiple="false"/>
				</td>
			</tr>
		...
		<div class="form-actions">
			...
			<c:if test="${ empty plmApplicationForLeave.procInsId}">
			...
			<c:if test="${not empty plmApplicationForLeave.id}">
			<a id="btnCancel" class="btn" href="javascript:;" onclick="history.go(-1)" ><i class="icon-reply"></i>返回</a>
			</c:if>
			<c:if test="${empty plmApplicationForLeave.id}">
