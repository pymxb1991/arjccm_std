...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/org/ccmOrgNpseFire/">数据列表</a></li>
		<%-- <shiro:hasPermission name="org:ccmOrgNpseFire:edit"><li><a href="${ctx}/org/ccmOrgNpseFire/form">消防安全重点添加</a></li></shiro:hasPermission> --%>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpseFire/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="second-line"><label>登记注册开始日期：</label>
				...
				<li class="second-line"><label>登记注册结束时间：</label><input name="endRegisterDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
...
<a
				onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseFire/form', '添加', '1100px', '720px')"
				class="btn btn-export" style="width: 49px;display:inline-block;float: right;"><i></i><span>添加</span> </a>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		<thead>
			<tr>
				...
				<th>单位负责人</th>
				<th>单位负责人联系方式</th>
				<shiro:hasPermission name="org:ccmOrgNpseFire:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="ccmOrgNpse">
			<tr>
				...
				<td>
					<a onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseFire/form?id=${ccmOrgNpse.id}', '编辑', '1100px', '800px')">${ccmOrgNpse.compName}</a>
<%--					<a href="${ctx}/org/ccmOrgNpseFire/form?id=${ccmOrgNpse.id}">--%>
<%--					${ccmOrgNpse.compName}</a>--%>
				...
				<td><shiro:hasPermission name="org:ccmOrgNpseFire:edit">
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseFire/form?id=${ccmOrgNpse.id}', '编辑', '1100px', '720px')"title="修改"><i class="icon-pencil"></i></a>
				<a class="btnList" href="${ctx}/org/ccmOrgNpseFire/delete?id=${ccmOrgNpse.id}" onclick="return confirmx('确认要删除该消防安全重点吗？', this.href)"  title="删除"><i class="icon-remove-sign"></i></a>
				</shiro:hasPermission> <shiro:hasPermission name="log:ccmLogTail:edit">
<%--				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseFire', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>--%>
				...
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npse', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>
