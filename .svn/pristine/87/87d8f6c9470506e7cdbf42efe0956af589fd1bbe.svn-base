...
<body>
	<ul class="nav nav-tabs">
		<li><a style="width: 140px;text-align:center" href="${ctx}/pop/ccmWork/list/20">数据列表</a></li>
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/pop/ccmWork/form/flow?id=${ccmPeople.id}">数据<shiro:hasPermission name="pop:ccmWork:edit">${not empty ccmPeople.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="pop:ccmWork:edit">查看</shiro:lacksPermission></a></li>
	...
	<form:form id="inputForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmWork/save/flow" method="post" class="form-horizontal">
		<form:hidden path="id"/>
