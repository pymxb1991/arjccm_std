...
<body>
	<ul class="nav nav-tabs">
		<li><a style="width: 140px;text-align:center" href="${ctx}/know/ccmKnowSpace/">数据列表</a></li>
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/know/ccmKnowSpace/form?id=${ccmKnowSpace.id}">数据<shiro:hasPermission name="know:ccmKnowSpace:edit">${not empty ccmKnowSpace.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="know:ccmKnowSpace:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="ccmKnowSpace" action="${ctx}/know/ccmKnowSpace/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
					...
					<div>
						...
						<div class="controls">
							<input name="relDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
								value="<fmt:formatDate value="${ccmKnowSpace.relDate}" pattern="yyyy-MM-dd"/>"
				...
				<td>
					<div>
						...
						<div class="controls">
							...
							value="<fmt:formatDate value="${ccmKnowSpace.impDate}" pattern="yyyy-MM-dd"/>"
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
						</div>
					</div>
				</td>
					...
					<div>
						...
						<div class="controls">
							<input name="ratifyDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
								value="<fmt:formatDate value="${ccmKnowSpace.ratifyDate}" pattern="yyyy-MM-dd"/>"
								onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
						</div>
					</div>
				</td>
		...
		<div class="form-actions">
			<shiro:hasPermission name="know:ccmKnowSpace:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
