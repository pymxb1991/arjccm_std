...
<body>
	<ul class="nav nav-tabs">
		<%-- 		<li class="active"><a
			href="${ctx}/education/ccmPlaceEducation/form?id=${ccmPlaceEducation.id}">研究院信息<shiro:hasPermission
					name="education:ccmPlaceEducation:edit">${not empty ccmPlaceEducation.id?'修改':'添加'}</shiro:hasPermission>
	...
	<form:form id="inputForm" modelAttribute="ccmPlaceEducation"
		action="${ctx}/education/ccmPlaceEducation/save/02" method="post"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceEducation.ccmBasePlace.administrativeDivision}" labelName=""
							labelValue="${ccmPlaceEducation.ccmBasePlace.area.name}" title="网格"
		...
		<h4 class="tableName">研究院信息</h4>
		<table>
			<tr>
				<td><label class="control-label">研究院办公电话：</label>
					<div class="controls">
						<form:input path="graduateSchoolPhone" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>
				<td><label class="control-label">研究院性质：</label>
					<div class="controls">
						<form:select path="graduateSchoolNature" class="input-xlarge">
							<form:options items="${fns:getDictList('ccm_graduate_property')}"
								itemLabel="label" itemValue="value" htmlEscape="false"
								class="required" />
						</form:select>
					</div></td>
