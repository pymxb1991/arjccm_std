			...
			.ready(
					function() {
						...
						$('#ccmBasePlaceCommon')
								.attr(
										'href',
										'${ctx}/place/ccmBasePlace/form/common?id=${ccmPlaceChemical.basePlaceId}&name='
												+ encodeURI('农贸市场信息')
												+ '&url=${ctx}/live/ccmPlaceChemical/form?id=${ccmPlaceChemical.id}')
...
<body>
	...
	<form:form id="inputForm" modelAttribute="ccmPlaceChemical"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceChemical.ccmBasePlace.administrativeDivision}" labelName=""
							labelValue="${ccmPlaceChemical.ccmBasePlace.area.name}" title="网格"
				...
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font></span>成立时间：</label>
					<div class="controls">
						<input name="ccmBasePlace.createTime" type="text"
							...
							value="<fmt:formatDate value="${ccmPlaceChemical.ccmBasePlace.createTime}" pattern="yyyy-MM-dd"/>"
		...
		<table>
			<tr>
				<td><label class="control-label">经营与可证编号：</label>
					<div class="controls">
						<form:input path="businessCertificateCode" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>
				<td><label class="control-label">经营许可证等级：</label>
					<div class="controls">
						<form:select path="businessCertificateLevel" class="input-xlarge">
							<form:option value="" label="请选择" />
							<form:options items="${fns:getDictList('ccm_place_chemical_business_icense_level')}"
								itemLabel="label" itemValue="value" htmlEscape="false"
								class="required" />
						</form:select>
					</div></td>
			...
			<tr>
				...
				<td><label class="control-label">经营许可证有效开始日期：</label>
					<div class="controls">
						<input name="businessCertificateBegin" type="text"
							readonly="readonly" maxlength="20" class="input-medium Wdate "
							value="<fmt:formatDate value="${ccmPlaceChemical.businessCertificateBegin}" pattern="yyyy-MM-dd"/>"
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});" />
					</div></td>
				<td><label class="control-label">经营许可证有效结束日期：</label>
					<div class="controls">
						<input name="businessCertificateEnd" type="text"
							readonly="readonly" maxlength="20" class="input-medium Wdate "
							value="<fmt:formatDate value="${ccmPlaceChemical.businessCertificateEnd}" pattern="yyyy-MM-dd"/>"
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});" />
					</div></td>
			</tr>
			<tr>
				<td><label class="control-label">经营范围：</label>
					<div class="controls">
						<form:input path="businessScope" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>
				<td><label class="control-label">注册地址：</label>
					<div class="controls">
			...
			<tr>
				<td><label class="control-label">仓储地址：</label>
					<div class="controls">
						<form:input path="storageAddress" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>
				<td></td>
			</tr>
