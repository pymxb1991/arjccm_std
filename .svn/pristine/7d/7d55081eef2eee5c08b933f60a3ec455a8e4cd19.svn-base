...
<body>
	<ul class="nav nav-tabs">
		<%-- 		<li class="active"><a
			href="${ctx}/hotel/ccmPlaceHotel/form?id=${ccmPlaceHotel.id}">家庭旅馆信息<shiro:hasPermission
					name="hotel:ccmPlaceHotel:edit">${not empty ccmPlaceHotel.id?'修改':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="hotel:ccmPlaceHotel:edit">查看</shiro:lacksPermission></a></li> --%>
	</ul>
	<form:form id="inputForm" modelAttribute="ccmPlaceHotel"
		action="${ctx}/hotel/ccmPlaceHotel/save/02" method="post"
		...
		<h4 class="tableName">家庭旅馆信息</h4>
		<table>
			<tr>
				<td colspan="2"><label class="control-label">卫生许可证等级：</label>
					<div class="controls">
