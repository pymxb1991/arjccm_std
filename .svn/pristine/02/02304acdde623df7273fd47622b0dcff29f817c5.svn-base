			...
			for (CcmHarmNationalSecurity HarmNationalSecurity : list) {
				try {
					...
					if(StringUtils.isBlank(HarmNationalSecurity.getSuspicionType()) ||
						...
						if(EntityTools.isEmpty(HarmNationalSecurity.getReportOffice())) {
							str.append("呈报单位信息错误;");
						}
						if(StringUtils.isBlank(HarmNationalSecurity.getSuperviseStatus())) {
							str.append("监管状态信息错误;");
						}
						if(StringUtils.isBlank(HarmNationalSecurity.getDangerLevel())) {
							str.append("危险程度信息错误;");
						}
						if(HarmNationalSecurity.getReportDate()==null) {
							str.append("呈报时间信息错误;");
						}
						if(StringUtils.isBlank(HarmNationalSecurity.getDiscoveryWay())) {
							str.append("发现途径信息错误;");
						}
						if(StringUtils.isBlank(HarmNationalSecurity.getIsCrimeHistory())) {
							str.append("有无犯罪史信息错误;");
						}
						str.append(")");
						failureMsg.append("<br/>危害国家安全人员名 " + HarmNationalSecurity.getName() + " 导入失败：必填项为空。"+str.toString());
						continue;
					}
