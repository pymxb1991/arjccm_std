...
<head>
	...
	<script type="text/javascript">
		var key, lastValue = "", nodeList = [], type = getQueryString("type", "${url}"),fontCss = getQueryString("fontCss", "${url}");
		if(fontCss == null || fontCss == "" || fontCss == undefined){
			fontCss = {};
		}else{			
			fontCss = eval('(' + fontCss + ')');
		}
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false,fontCss:fontCss},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(type==3||type==4),url:((type==3)?"${ctx}/sys/user/treeData":"${ctx}/person/pbsDepartmentbind/treeData")  ,autoParam:["id=officeId"]},
