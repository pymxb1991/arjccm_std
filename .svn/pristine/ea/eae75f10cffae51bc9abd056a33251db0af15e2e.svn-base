...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/warning/ccmEarlyWarning/importantList">重点人员感知</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmEarlyWarning" action="${ctx}/warning/ccmEarlyWarning/importantList" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="ccmEarlyWarning">
			<tr>
				...
				<shiro:hasPermission name="warning:ccmEarlyWarning:edit"><td>
					...
					<a href="${ctx}/warning/ccmEarlyWarning/deleteImportantList?id=${ccmEarlyWarning.id}" onclick="return confirmx('确认要删除该人员记录吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
