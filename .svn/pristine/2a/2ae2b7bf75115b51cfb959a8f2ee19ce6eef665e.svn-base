...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/resource/plmResource/?type=01">资源共享列表</a></li>
		<li class="active"><a
			href="${ctx}/resource/plmResource/form?id=${plmResource.id}">资源共享<shiro:hasPermission
					name="resource:plmResource:edit">${not empty plmResource.id?'修改':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="resource:plmResource:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="plmResource" style="margin: 0px 260px;"
		action="${ctx}/resource/plmResource/save" method="post"
