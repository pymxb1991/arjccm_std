...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/casino/ccmPlaceCasino/01">数据列表</a></li>
	</ul>

	<form:form id="searchForm" modelAttribute="ccmPlaceCasino"
		action="${ctx}/casino/ccmPlaceCasino/01" method="post"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceCasino">
				<tr>
					...
					<td><a class="btnList"
						onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')">${ccmPlaceCasino.placeName}</a></td>
					...
					<td>${fns:getDictLabel(ccmPlaceCasino.isFree, 'ccm_place_free', '')}</td>
					...
					<shiro:hasPermission name="casino:ccmPlaceCasino:edit">
						...
						<a class="btnList"
							onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')"
