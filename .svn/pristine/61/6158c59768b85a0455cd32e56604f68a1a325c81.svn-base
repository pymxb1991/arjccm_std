...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="${ctx}/policy/plmKnowPolicyAct/?types=2">政策法规管理列表</a></li>
		<shiro:hasPermission name="policy:plmKnowPolicyAct:edit">
			<li><a href="${ctx}/policy/plmKnowPolicyAct/form">政策法规添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="plmKnowPolicy"
		action="${ctx}/policy/plmKnowPolicyAct/" method="post"
		...
		<div class="row">
			<c:forEach items="${page.list}" var="plmKnowPolicy">
				<div class="span4 spandiv">
					<div class="thumbnail"> 
						<a href="${ctx}/policy/plmKnowPolicyAct/one?id=${plmKnowPolicy.id}"><h4 title="${plmKnowPolicy.name}">标题:${plmKnowPolicy.name}</h4>
						</a>
						<div class="caption row-fluid">
							...
							<div class="spantext  " style="width: 90%; margin-left: 5%">
								<p title="${plmKnowPolicy.relDept}">发布部门:${plmKnowPolicy.relDept}</p>
								<p title="${plmKnowPolicy.lssNo}">发文字号:${plmKnowPolicy.lssNo}</p>
								<p title="<fmt:formatDate value="${plmKnowPolicy.updateDate}"
							pattern="yyyy-MM-dd" />">发布时间：<fmt:formatDate value="${plmKnowPolicy.updateDate}"
