...
<body>
	...
	<form:form id="searchForm" modelAttribute="ccmPartyOrganiz" action="${ctx}/partybuild/ccmPartyOrganiz/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><a
					onclick="parent.parent.parent.LayerDialog('${ctx}/partybuild/ccmPartyOrganiz/form?type=2', '添加', '1330px', '800px')"
		...
		<c:forEach items="${page.list}" var="ccmPartyOrganiz">
			<tr>
				...
				<shiro:hasPermission name="partybuild:ccmPartyOrganiz:edit"><td>
    				...
    				<%--<a href="${ctx}/partybuild/ccmPartyOrganiz/form?id=${ccmPartyOrganiz.id}">修改</a>--%>
					<%--<a href="${ctx}/partybuild/ccmPartyOrganiz/delete?id=${ccmPartyOrganiz.id}" onclick="return confirmx('确认要删除该党组织管理吗？', this.href)">删除</a>--%>
