...
public class PlmRoomReceptionApplyService extends CrudService<PlmRoomApplyDao, PlmRoomApply> {
	@Autowired
	...
	public void save(PlmRoomApply plmRoomApply) {
		...
		plmActService.save(plmRoomApply.getPlmAct(),PlmTypes.ROOM_APPLY_RECEPTION_ID,plmRoomApply.getId());
	}
	...
	public void apply(PlmRoomApply plmRoomApply) {
		...
		if (StringUtils.isBlank(plmRoomApply.getProcInsId())) {
			...
			Map<String, String> returnMap = actUtConfigService.getProcInsId(PlmTypes.ROOM_APPLY_RECEPTION_ID, plmRoomApply, plmRoomApply.getId());
			//3、更新业务表流程实例ID
			...
			plmActService.save(plmRoomApply.getPlmAct(),PlmTypes.ROOM_APPLY_RECEPTION_ID,plmRoomApply.getId(),plmRoomApply.getProcInsId());
		} else {
	...
	public void auditSave(PlmRoomApply plmRoomApply) {
		...
		if ("auditEnd".equals(taskDefKey)) {							
			...
			for (PlmRoomAttendee plmRoomAttendee : list) {
				plmCalendarService.quickAdd(plmRoomApplyDB.getStartTime(), plmRoomApplyDB.getEndTime(), plmRoomApplyDB.getSubject(), plmRoomAttendee.getUser(),"2");
			}
		}
		// 最后一步流程  不需要审核
