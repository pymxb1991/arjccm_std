...
import com.arjjs.ccm.modules.act.dao.ActDao;
import com.arjjs.ccm.modules.act.entity.Act;
import com.arjjs.ccm.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.arjjs.ccm.modules.act.service.cmd.JumpTaskCmd;
import com.arjjs.ccm.modules.act.service.creator.ChainedActivitiesCreator;
import com.arjjs.ccm.modules.act.service.creator.MultiInstanceActivityCreator;
import com.arjjs.ccm.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.arjjs.ccm.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.arjjs.ccm.modules.act.utils.ActUtils;
import com.arjjs.ccm.modules.act.utils.ProcessDefCache;
import com.arjjs.ccm.modules.act.utils.ProcessDefUtils;
...
public class ActTaskService extends BaseService {
	 ...
	 * @return
	 */
	public List<Act> todoList(Act act,String userName){
		String userId = null;
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	 ...
	 * @return
	 */
	public Page<Act> historicList(Page<Act> page, Act act,String userName){
		String userId = null;
		if (userName != null && !"".equals(userName)) {
			userId = userName;
		} else {
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	 ...
	 * @param userName		用户登录名 （pengjianqiang：手机app流程提交时，无法从UserUtils中取得，所以增加参数）
	 * @return 流程实例ID
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, String userName) {
		Map<String, Object> vars = Maps.newHashMap();
		return startProcess(procDefKey, businessTable, businessId, title, vars, userName);
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars, String userName) {
		if (userName != null && !"".equals(userName)) {
		...
		// 配置部门主负责人
		User user = userDao.get(UserUtils.getUser().getId());
		if (null != user.getOffice() && null != user.getOffice().getPrimaryPerson()) {
			vars.put("primaryPerson", UserUtils.get(user.getOffice().getPrimaryPerson().getId()).getLoginName());
		}

		// 配置部门副负责人
		if (null != user.getOffice() && null != user.getOffice().getDeputyPerson()) {
			vars.put("deputyPerson", UserUtils.get(user.getOffice().getDeputyPerson().getId()).getLoginName());
		}
...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	...
	public  String histoicTable(String procInsId,  String colspan,  String titleColspan){

		String histoicFlow="";

		if (StringUtils.isNotBlank(procInsId)){
			List<Act> histoicFlowList = histoicFlowList(procInsId, "", "");


			SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
			String endTime="";
			for (Act act : histoicFlowList) {
				if(act.getHistIns().getEndTime()!=null) {
					endTime=simpleDateFormat.format(act.getHistIns().getEndTime());
				}
				if(act.getHistIns().getActivityId().equals("start")) {
				}else {
					if(endTime.equals("")) {
						histoicFlow+="<tr>" +
								"			<td class='trtop shenpi' colspan='"+titleColspan+"'>"+act.getHistIns().getActivityName()+"</td>" +
								"			<td class='shenpi' style='text-align:left;' colspan='"+colspan+"'> " +
								"			" +
								"			进行中.... <br /><br/><div style='float: right;'>  </div>" +
								"			</td>" +
								"			" +
								"		</tr>";
					}else {
						histoicFlow+="<tr>" +
								...
								"			<td class='shenpi' style='text-align:left;' colspan='"+colspan+"'> " +
								"			" +
								"			审批意见："+act.getComment()+"<br/><div style='float: right;'><div style='text-align:left; width: 190px'> 审批人："+act.getAssigneeName()+"  <br/> 日期："+endTime+"</div> </div>" +
								"			</td>" +
								"			" +
								"		</tr>";
