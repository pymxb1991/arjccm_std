...
<body>
	<ul class="nav nav-tabs">
		<%-- 		<li class="active"><a
			href="${ctx}/hospital/ccmPlaceHospital/form?id=${ccmPlaceHospital.id}">药店信息<shiro:hasPermission
	...
	<form:form id="inputForm" modelAttribute="ccmPlaceHospital"
		action="${ctx}/hospital/ccmPlaceHospital/save/03" method="post"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceHospital.ccmBasePlace.administrativeDivision}"
							labelName=""
							labelValue="${ccmPlaceHospital.ccmBasePlace.area.name}"
		...
		<h4 class="tableName">药店信息</h4>
		<table>
			<tr>

				<td colspan="2"><label class="control-label"><span class="help-inline"><font color="red" id="show1">*</font></span>证照情况：</label>
					<div class="controls">
						<form:checkboxes path="LicenceList"
							items="${fns:getDictList('ccm_hospital_licence')}"
							itemLabel="label" itemValue="value" htmlEscape="false"
							class="required" />
			...
			<tr>
				<td><label class="control-label">注册资金：</label>
					<div class="controls">
						<form:input path="registeredFund" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>

				<td><label class="control-label">药师人数：</label>
					<div class="controls">
						<form:input path="pharmacistNumber" htmlEscape="false"
							maxlength="255" class="input-xlarge " />
					</div></td>
			</tr>
