...
<div class="context" content="${ctx}"></div>
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatHeresy">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseHeresy" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHouseHeresy" action="${ctx}/house/ccmHouseHeresy/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>教派名称：</label> <form:input path="heresyName" htmlEscape="false" maxlength="64" class="input-medium" />
			</li>
			<!--  
			<li><label>痴迷程度：</label>
			...
			-->
			<li class="first-line"><label>是否已转化：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<c:forEach items="${page.list}" var="ccmHouseHeresy">
			<tr>
				...
				<td class="tc">
				<a onclick="parent.LayerDialog('${ctx}/house/ccmHouseHeresy/form?id=${ccmHouseHeresy.id}', '信息', '1200px', '900px')">${ccmHouseHeresy.name} </a></td>
				<td class="tc">${ccmHouseHeresy.ident}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseHeresy.sex, 'sex', '')}</td>
				...
				<td class="tc">${ccmHouseHeresy.heresyName}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseHeresy.level, 'ccm_house_heresy_level', '')}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseHeresy.isStudy, 'yes_no', '')}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseHeresy.isChange, 'yes_no', '')}</td>
				<td class="tc"><shiro:hasPermission name="house:ccmHouseHeresy:edit">
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseHeresy/form?id=${ccmHouseHeresy.id}', '修改', '1200px', '900px')" title="修改"><i class="icon-pencil"></i></a>
					<a class="btnList" href="${ctx}/house/ccmHouseHeresy/delete?id=${ccmHouseHeresy.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该涉教人员吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseHeresy.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseHeresy.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="icon-group"></i></a>
				...
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseHeresy.id}&relevance_table=ccm_house_heresy', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseHeresy.id}&relevance_table=ccm_house_heresy', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>
