...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/exam/pbsExampaper/?sWay=0">在线试卷列表</a></li>
		<shiro:hasPermission name="exam:pbsExampaper:edit">
			<li><a href="${ctx}/exam/pbsExampaper/formOnline">在线试卷添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="pbsExampaper"
		action="${ctx}/exam/pbsExampaper/?sWay=0" method="post"
				...
				<tr>
					...
					<shiro:hasPermission name="exam:pbsExampaper:edit">
						<td><a
							...
							href="${ctx}/exam/pbsExampaper/paperDetail?id=${pbsExampaper.id}&sWay=0" title = "查看"><i class="icon-eye-open"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
