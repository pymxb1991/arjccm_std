...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/chemical/ccmPlaceChemical/">数据列表</a></li>
	</ul>
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceChemical">
				<tr>
					<td width="200px"><img
						src="${ccmPlaceChemical.ccmBasePlace.placePicture}"
						 class="pic-size pimg"/></td>
					<td><a class="btnList"
						onclick="parent.LayerDialog('${ctx}/chemical/ccmPlaceChemical/form?id=${ccmPlaceChemical.id}', '编辑', '1200px', '900px')">${ccmPlaceChemical.placeName}</a></td>
					...
					<td>${ccmPlaceChemical.businessCertificateCode}</td>
					<td>${fns:getDictLabel(ccmPlaceChemical.businessCertificateLevel, 'ccm_place_chemical_business_icense_level', '无')}</td>
					<td><fmt:formatDate value="${ccmPlaceChemical.createDate}"
					...
					<shiro:hasPermission name="chemical:ccmPlaceChemical:edit">
						...
						<a class="btnList"
							...
							onclick="return confirmx('确认要删除该危化品经营场所吗？', this.href)"
