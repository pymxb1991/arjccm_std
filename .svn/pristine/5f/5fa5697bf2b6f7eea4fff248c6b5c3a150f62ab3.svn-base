...
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseEscape/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
			<input id="uploadFile" name="file" type="file" style="width: 330px" /><br /> <br />
			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="导  入 " />
		</form>
	</div>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatEscape">数据统计</a></li>
		<li class="active"><a href="javascript:;" data-href="${ctx}/house/ccmHouseEscape/" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseEscape">
				<tr>
					...
					<td><a onclick="parent.LayerDialog('${ctx}/house/ccmHouseEscape/form?id=${ccmHouseEscape.id}', '信息', '1200px', '900px')">${ccmHouseEscape.name}</a></td>
					...
					<td><shiro:hasPermission name="house:ccmHouseEscape:edit">
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseEscape/form?id=${ccmHouseEscape.id}', '修改', '1200px', '900px')" title="修改"><i class="icon-pencil"></i></a>
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseEscape.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="icon-group"></i></a>
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseEscape.id}&relevance_table=ccm_house_escape', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseEscape.id}&relevance_table=ccm_house_escape', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>
