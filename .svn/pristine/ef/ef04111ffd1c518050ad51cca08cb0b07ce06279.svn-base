...
<body>
	<ul class="nav nav-tabs">
		<%-- <li><a href="${ctx}/org/ccmOrgNpseSecurity/">数据列表</a></li>
		<li class="active"><a href="${ctx}/org/ccmOrgNpseSecurity/form?id=${ccmOrgNpse.id}">数据<shiro:hasPermission name="org:ccmOrgNpseSecurity:edit">${not empty ccmOrgNpse.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="org:ccmOrgNpseSecurity:edit">查看</shiro:lacksPermission></a></li> --%>
		<%-- <c:if test="${not empty ccmOrgNpse.id}">
			<li><a
				href="${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseSecurity">跟踪信息<shiro:hasPermission
