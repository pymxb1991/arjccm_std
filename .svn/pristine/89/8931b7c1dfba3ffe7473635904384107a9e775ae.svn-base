...
<div class="back-list">
	...
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseDispute/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
	...
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="house:ccmHouseDispute:edit"><li><a href="${ctx}/house/ccmHouseDispute/form">闹事行凶报复嫌疑添加</a></li></shiro:hasPermission> --%>
			<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatDispute">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseDispute" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHouseDispute" action="${ctx}/house/ccmHouseDispute/" method="post" class="breadcrumb form-search clearfix">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<c:forEach items="${page.list}" var="ccmHouseDispute">
			<tr>
				...
				<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHouseDispute/form?id=${ccmHouseDispute.id}', '信息', '1200px', '900px')">${ccmHouseDispute.name}</a></td>
				<td class="tc">${ccmHouseDispute.ident}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseDispute.sex, 'sex', '')}</td>
				<td class="tc">${ccmHouseDispute.telephone}</td>
				<td class="tc">${fns:getDictLabel(ccmHouseDispute.isCrimeHistory, 'yes_no', '')}</td>
				...
				<td class="tc"><shiro:hasPermission name="house:ccmHarmNationalSecurity:edit">
					...
					<a class="btnList" href="${ctx}/house/ccmHouseDispute/delete?id=${ccmHouseDispute.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除闹事行凶报复嫌疑人口吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
