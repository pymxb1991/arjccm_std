...
<div class="context" content="${ctx}"></div>
	<ul class="nav nav-tabs">
		<li class="active" ><a class="nav-head" href="${ctx}/event/ccmEventIncident/listStudent">师生案（事）件列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmEventIncident"
		action="${ctx}/event/ccmEventIncident/listStudent" method="post"
		class="breadcrumb form-search clearfix">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden"
			value="${page.pageSize}" />
		<ul class="ul-form pull-left">
			<li class="first-line"><label class="title-text">案（事）件名称：</label> <form:input path="caseName"
			...
			<li class="first-line"><label class="title-text">发生日期：</label> <input name="beginHappenDate" type="text" readonly="readonly"
											 maxlength="20" class="input-medium Wdate"
											 value="<fmt:formatDate value="${ccmEventIncident.beginHappenDate}" pattern="yyyy-MM-dd HH:mm:ss"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});" /></li>
			<li class="first-line"><label class="title-text">结束日期：</label> <input name="endHappenDate" type="text" readonly="readonly"
											maxlength="20" class="input-medium Wdate"
											value="<fmt:formatDate value="${ccmEventIncident.endHappenDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});" />
			</li>
			<li class="first-line"><label class="title-text">发案地：</label>
			...
			<li class="first-line"><label class="title-text">案（事）件分级：</label> <form:select path="eventScale"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_case_grad')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="second-line"><label class="title-text">案（事）件类型：</label> <form:select path="eventType"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('ccm_case_type')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="second-line"><label class="title-text">案（事）件性质：</label>
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmEventIncident">
				<tr>
					...
					<td><a href="${ctx}/event/ccmEventIncident/formStudent?id=${ccmEventIncident.id}">
					...
					<td>
						<!-- 事件登记编辑权限  --> <shiro:hasPermission
							...
							<a  class="btnList"
								href="${ctx}/event/ccmEventIncident/formStudent?id=${ccmEventIncident.id}" title="修改"><i class="icon-pencil"></i></a>
							<a	class="btnList"
								href="${ctx}/event/ccmEventIncident/deleteStudent?id=${ccmEventIncident.id}"
							...
							<!-- 
							<a class="btnList" href="${ctx}/event/ccmEventCasedeal/dealformStudent?eventIncidentId=${ccmEventIncident.id}" title="添加处理信息"><i class="icon-plus"></i></a>
							 -->
