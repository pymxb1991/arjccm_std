...
<body>
	<ul class="nav nav-tabs">
		<%-- <li><a href="${ctx}/org/ccmOrgNpseLog/">物流安全重点列表</a></li> 
		<li class="active"><a href="${ctx}/org/ccmOrgNpseLog/form?id=${ccmOrgNpse.id}">物流安全重点<shiro:hasPermission name="org:ccmOrgNpseLog:edit">${not empty ccmOrgNpse.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="org:ccmOrgNpseLog:edit">查看</shiro:lacksPermission></a></li>--%>
		<%-- <c:if test="${not empty ccmOrgNpse.id}">
			<li><a
				href="${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseLog">跟踪信息<shiro:hasPermission
	...
	<form:form id="inputForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpseLog/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
			...
			<tr>
				...
				<td>
					<div>
						...
						<div class="controls">
							<form:input path="compArea" htmlEscape="false" class="input-xlarge number" />
						</div>
					</div>
				</td>
			</tr>
			...
			<tr>
				<td>
					<div>
						...
						<div class="controls">
							<form:input path="entePrinName" htmlEscape="false" maxlength="50" class="input-xlarge"/>
						</div>
					</div>
				</td>
				<td>
			...
			<tr>
				...
				<td>
					<div>
						...
						<div class="controls">
							<form:select path="enteType" class="input-xlarge ">
								<form:options items="${fns:getDictList('ccm_comp_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						    </form:select>
						</div>
					</div>
				</td>
			...
			<tr>
				...
				<td>
					<div>
						...
						<div class="controls">
							<form:input path="companyNum" htmlEscape="false" maxlength="6" class="input-xlarge number digits "/>
						</div>
					</div>
				</td>
			</tr>
