...
<ul class="back-list">
	...
	<form:form id="searchForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpse/?compType=05" method="post" class="breadcrumb form-search clearfix">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form pull-left">
			<li class="first-line"><label>社团名称：</label>
				<form:input path="compName" htmlEscape="false" maxlength="100" class="input-medium"/>
			</li>
			<li class="first-line"><label>登记注册开始日期：</label>
			...
			<li class="first-line"><label>登记注册结束日期：</label>
...
<a
				onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpse/formData?compType=05', '添加', '1100px', '800px')"
		...
		<c:forEach items="${page.list}" var="ccmOrgNpse">
			<tr>
				...
				<td><shiro:hasPermission name="org:ccmOrgNpse:edit">
					...
					<a class="btnList" href="${ctx}/org/ccmOrgNpse/delete?id=${ccmOrgNpse.id}" onclick="return confirmx('确认要删除该社团组织吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
				</shiro:hasPermission> 
			</tr>
		</c:forEach>
		</tbody>
