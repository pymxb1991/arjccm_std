...
<div class="context" content="${ctx}"></div>
 <!-- 导入、导出模块 -->
	...
	<ul class="nav nav-tabs">
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/pop/ccmPermanent/">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmPermanent/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>姓名：</label>
				<form:input path="name" htmlEscape="false" maxlength="50" class="input-medium"  cssStyle="width:160px"/>
			<li/>
		...
		<li class="clearfix selectHidden hide"></li>
			...
			<li class="selectHidden hide"><label style="width: 120px" >出生结束时间</label>	<input name="endBirthday" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>所属网格</th>
				<th>现住门（楼）详址</th>
				<shiro:hasPermission name="pop:ccmPermanent:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="ccmPeople">
			<tr>
				...
				<td class="tc"><a href="${ctx}/pop/ccmPermanent/form?id=${ccmPeople.id}">
					${ccmPeople.name}</a>
				</td>
				<td class="tc">
				...
				<td class="tc"><shiro:hasPermission name="pop:ccmPermanent:edit">
	    				<a class="btnList"  href="${ctx}/pop/ccmPermanent/form?id=${ccmPeople.id}"  title="修改"><i class="icon-pencil"></i></a>
						<a class="btnList" href="${ctx}/pop/ccmPermanent/delete?id=${ccmPeople.id}" onclick="return confirmx('确认要删除该实有人口吗？', this.href)"  title="删除"><i class="icon-remove-sign"></i></a>
				   <a class="btnList"
