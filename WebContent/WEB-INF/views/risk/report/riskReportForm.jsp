...
<body>
	...
	<form:form id="inputForm" modelAttribute="riskReport" action="${ctx}/report/riskReport/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<shiro:hasPermission name="report:riskReport:edit"><input id="btnSubmit" class="btn btn-primary" onclick="saveForm()" type="button"  value="保 存"/>&nbsp;</shiro:hasPermission>
			<c:if test="${not empty riskReport.id}">
				<a class="btn btn-success" href="${ctx}/report/riskReport/saveList?id=${riskReport.id}" onclick="return confirmx('确认要将该事项评估报告入库吗？', this.href)">入库</a> 
			</c:if>
