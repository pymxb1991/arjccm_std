...
<body>
	<ul class="nav nav-tabs">
		<%-- <li><a href="${ctx}/org/ccmOrgNpseFire/">消防安全重点列表</a></li> 
		<li class="active"><a href="${ctx}/org/ccmOrgNpseFire/form?id=${ccmOrgNpse.id}">消防安全重点<shiro:hasPermission name="org:ccmOrgNpseFire:edit">${not empty ccmOrgNpse.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="org:ccmOrgNpseFire:edit">查看</shiro:lacksPermission></a></li> --%>
   <%-- <c:if test="${not empty ccmOrgNpse.id}">
	...
	<form:form id="inputForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpseFire/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
