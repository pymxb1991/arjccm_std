...
<body>
	<ul class="nav nav-tabs">
		<%-- 		<li class="active"><a href="${ctx}/hotel/ccmPlaceHotel/form?id=${ccmPlaceHotel.id}">酒店住宿<shiro:hasPermission name="hotel:ccmPlaceHotel:edit">${not empty ccmPlaceHotel.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="hotel:ccmPlaceHotel:edit">查看</shiro:lacksPermission></a></li>
 --%>
	</ul>

	<form:form id="inputForm" modelAttribute="ccmPlaceHotel"
		action="${ctx}/hotel/ccmPlaceHotel/save/01" method="post"
				...
				<td><label class="control-label" style="display: none">场所类型：</label>
					...
					<div class="controls">
						<sys:treeselect id="area"
							...
							value="${ccmPlaceHotel.ccmBasePlace.administrativeDivision}" labelName=""
							labelValue="${ccmPlaceHotel.ccmBasePlace.area.name}" title="网格"
				...
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font></span>成立时间：</label>
					<div class="controls">
						<input name="ccmBasePlace.createTime" type="text"
							...
							value="<fmt:formatDate value="${ccmPlaceHotel.ccmBasePlace.createTime}" pattern="yyyy-MM-dd"/>"
		...
		<h4 class="tableName">酒店信息</h4>
		<table>
			<tr>
				<td><label class="control-label"><span class="help-inline"><font color="red">*</font> </span>酒店星级：</label>
					<div class="controls">
						<form:select path="hotelRank" class="input-xlarge">
							<form:options items="${fns:getDictList('ccm_hotel_level')}"
								itemLabel="label" itemValue="value" htmlEscape="false"
								class="required" />
						</form:select>
					</div></td>
				<td><label class="control-label">卫生许可证等级：</label>
					<div class="controls">
						<form:select path="hygienicLicenseRank" class="input-xlarge">
							<form:options items="${fns:getDictList('ccm_licence_level')}"
								itemLabel="label" itemValue="value" htmlEscape="false"
								class="required" />
						</form:select>
					</div></td>
