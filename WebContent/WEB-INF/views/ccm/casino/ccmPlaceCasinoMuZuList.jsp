...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/casino/ccmPlaceCasino/08">数据列表</a></li>
	</ul>

	<form:form id="searchForm" modelAttribute="ccmPlaceCasino"
		action="${ctx}/casino/ccmPlaceCasino/08" method="post"
...
<%--			<li class="clearfix"></li>--%>
	...
	<div class="clearfix pull-right btn-box">
		<a
				onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=08', '添加', '1100px', '700px')"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceCasino">
				<tr>
					...
					<td><a class="btnList"
						onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=08&id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')">${ccmPlaceCasino.placeName}</a></td>
					...
					<shiro:hasPermission name="casino:ccmPlaceCasino:edit">
						<td><a class="btnList"
							onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=08&id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')"
							title="修改"><i class="icon-pencil"></i></a> <a class="btnList"
							href="${ctx}/casino/ccmPlaceCasino/delete?id=${ccmPlaceCasino.id}"
							onclick="return confirmx('确认要删除该沐足场所吗？', this.href)" title="删除"><i
