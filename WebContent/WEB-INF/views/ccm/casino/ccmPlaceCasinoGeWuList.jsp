...
<div class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/casino/ccmPlaceCasino/03">数据列表</a></li>
	</ul>

	<form:form id="searchForm" modelAttribute="ccmPlaceCasino"
		action="${ctx}/casino/ccmPlaceCasino/03" method="post"
...
<%--			<li class="clearfix"></li>--%>
	...
	<div class="clearfix pull-right btn-box">
		<a
				onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=03', '添加', '1100px', '700px')" style="width: 49px;display:inline-block;float: right;"
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceCasino">
				<tr>
					<td width="200px"><img src="${ccmPlaceCasino.ccmBasePlace.placePicture}"
						style="height: 50px;" class="pimg"/></td>
					<td><a class="btnList"
						onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=03&id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')">${ccmPlaceCasino.placeName}</a></td>
					<td>${ccmPlaceCasino.leaderName}</td>
					<td>${ccmPlaceCasino.leaderContact}</td>
					<td>${fns:getDictLabel(ccmPlaceCasino.relevanceOrg, 'ccm_buss_cate', '无')}</td>
					<td>${fns:getDictLabel(ccmPlaceCasino.isLicense, 'yes_no', '未知')}</td>
					<td><fmt:formatDate value="${ccmPlaceCasino.updateDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<shiro:hasPermission name="casino:ccmPlaceCasino:edit">
						<td><a class="btnList"
							onclick="parent.LayerDialog('${ctx}/casino/ccmPlaceCasino/form?type=03&id=${ccmPlaceCasino.id}', '编辑', '1100px', '700px')"
							title="修改"><i class="icon-pencil"></i></a> <a class="btnList"
							href="${ctx}/casino/ccmPlaceCasino/delete?id=${ccmPlaceCasino.id}"
							onclick="return confirmx('确认要删除该歌舞厅场所吗？', this.href)" title="删除"><i
