...
<%--<span class="nav-position">当前位置 ：</span><span class="nav-menu"><%=session.getAttribute("activeMenuName")%>></span><span class="nav-menu2">日常工作</span>--%>
<div class="back-list">
<input type="hidden" id="hasPermission" value="${fns:getUser().hasPermission}"/>
<div class="context" content="${ctx}"></div>
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseAids/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseAids">
				<tr>
					...
					<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHouseAids/form?id=${ccmHouseAids.id}', '信息', '1200px', '900px')">${ccmHouseAids.name}</a></td>
					<td class="tc">${ccmHouseAids.ident}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseAids.sex, 'sex', '')}</td>
					<td class="tc">${ccmHouseAids.telephone}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseAids.crimPast, 'yes_no', '')}</td>
					...
					<td class="tc">${fns:getDictLabel(ccmHouseAids.treaCase, 'ccm_tred_case', '')}
					</td>
					<td class="tc"><shiro:hasPermission name="house:ccmHouseAids:edit">
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseAids/form?id=${ccmHouseAids.id}', '修改', '1200px', '900px')" title="修改"><i class="icon-pencil"></i></a>
						<a class="btnList" href="${ctx}/house/ccmHouseAids/delete?id=${ccmHouseAids.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该艾滋病危险人员吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
						<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseAids.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseAids.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="icon-group"></i></a>
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseAids.id}&relevance_table=ccm_house_aids', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseAids.id}&relevance_table=ccm_house_aids', '添加记录', '800px', '660px')" title="添加记录"><i class="icon-plus"></i></a>
