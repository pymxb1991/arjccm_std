...
<div class="back-list">
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatSeriousCriminalOffense">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmSeriousCriminalOffense" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmSeriousCriminalOffense" action="${ctx}/house/ccmSeriousCriminalOffense/" method="post" class="breadcrumb form-search clearfix">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmSeriousCriminalOffense">
				<tr>
					<td width="100px"><img src="${ccmSeriousCriminalOffense.images}" style="height:50px;" class="pimg"/></td>
					<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmSeriousCriminalOffense/form?id=${ccmSeriousCriminalOffense.id}', '信息', '1200px', '900px')">${ccmSeriousCriminalOffense.name}</a></td>
					<td class="tc">${ccmSeriousCriminalOffense.ident}</td>
					<td class="tc">${fns:getDictLabel(ccmSeriousCriminalOffense.sex, 'sex', '')}</td>
					<td class="tc">${ccmSeriousCriminalOffense.telephone}</td>
					...
					<td class="tc">${fns:getDictLabel(ccmSeriousCriminalOffense.isCrimeHistory, 'have_no', '')}</td>
					<td class="tc">${fns:getDictLabel(ccmSeriousCriminalOffense.suspicionType, 'criminal_type', '')}
					</td>
					<td class="tc">
						<shiro:hasPermission name="house:ccmSeriousCriminalOffense:edit">
							...
							onclick="return confirmx('确认要删除该严重刑事犯罪人口吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
							<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmSeriousCriminalOffense.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
						...
						<shiro:hasPermission name="log:ccmLogTail:edit">
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmSeriousCriminalOffense.id}&relevance_table=ccm_serious_criminal_offense', '记录信息', '800px', '660px')" 
