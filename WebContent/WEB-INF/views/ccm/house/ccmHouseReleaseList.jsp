...
<div class="context" content="${ctx}"></div>
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatRelease">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseRelease" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHouseRelease" action="${ctx}/house/ccmHouseRelease/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="second-line"><label >释放开始日期：</label>
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseRelease">
				<tr>
					...
					<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHouseRelease/form?id=${ccmHouseRelease.id}', '信息', '1200px', '900px')">${ccmHouseRelease.name}</a></td>
					<td class="tc">${ccmHouseRelease.ident}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseRelease.sex, 'sex', '')}</td>
					<td class="tc">${ccmHouseRelease.telephone}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseRelease.recidivism, 'yes_no', '')}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseRelease.origCha, 'ccm_chag_type', '')}</td>
					...
					<td class="tc"><fmt:formatDate value="${ccmHouseRelease.releDate}" pattern="yyyy-MM-dd" /></td>
					<td class="tc"><shiro:hasPermission name="house:ccmHouseRelease:edit">
					...
					<a class="btnList" href="${ctx}/house/ccmHouseRelease/delete?id=${ccmHouseRelease.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该安置帮教人员吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseRelease.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
					...
					<%-- <a class="btnList" onclick="parent.LayerDialog1('','${ctx}/work/ccmWorkTiming/form', '定时提醒', '700px', '500px')" title="定时提醒"><i class="icon-bell"></i></a> --%>	  
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseRelease.id}&relevance_table=ccm_house_release', '记录信息', '800px', '660px')" 
