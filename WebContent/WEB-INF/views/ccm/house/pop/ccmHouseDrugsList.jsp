...
<script type="text/javascript"
	      src="${ctxStatic}/ccm/house/js/ccmHouseDrugsInfo.js">
	</script>
	<script type="text/javascript"
...
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseDrugs/import"
	...
	<form:form id="searchForm" modelAttribute="ccmHouseDrugs"
		action="${ctx}/house/ccmHouseDrugs/" method="post"
		...
		<ul class="ul-form">
			...
			<li><label >关注程度：</label> <form:select path="atteType"
					class="input-medium">
					<form:option value="" label="全部" />
	...
	<!-- 导入、导出模块 -->
			...
			<li><label>吸毒原因：</label> <form:select
					path="drugCaus" class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('sys_ccm_drugs_caus')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li><label>有无犯罪史：</label> <form:select path="crimPast"
					class="input-medium">
					<form:option value="" label="全部" />
					<form:options items="${fns:getDictList('have_no')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseDrugs">
				<tr>
					<td width="100px">
						<img src="${ccmHouseDrugs.images}" style="height:50px;" class="pimg"/>
					</td>
					<td>
						<a onclick="parent.LayerDialog('${ctx}/house/ccmHouseDrugs/form?id=${ccmHouseDrugs.id}', '信息', '1200px', '900px')">${ccmHouseDrugs.name}</a>
					</td>
					...
					<td>${fns:getDictLabel(ccmHouseDrugs.drugCaus, 'sys_ccm_drugs_caus', '')}
					</td>
					 
					<td><shiro:hasPermission name="house:ccmHouseDrugs:edit">
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseDrugs/form?id=${ccmHouseDrugs.id}', '修改', '1200px', '900px')"
							...
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseDrugs.id}&relevance_table=ccm_house_drugs', '添加记录', '800px', '660px')"
