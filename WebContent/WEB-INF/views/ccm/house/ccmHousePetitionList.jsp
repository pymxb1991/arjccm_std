...
<div class="context" content="${ctx}"></div>
	...
	<form:form id="searchForm" modelAttribute="ccmHousePetition" action="${ctx}/house/ccmHousePetition/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>上访性质：</label>
			...
			<li class="first-line"><label>个或群访：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<c:forEach items="${page.list}" var="ccmHousePetition">
			<tr>
				...
				<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHousePetition/form?id=${ccmHousePetition.id}', '信息', '1200px', '900px')">${ccmHousePetition.name}</a></td>
				<td class="tc">${ccmHousePetition.ident}</td>
				<td class="tc">${fns:getDictLabel(ccmHousePetition.sex, 'sex', '')}</td>
				...
				<shiro:hasPermission name="house:ccmHousePetition:edit">
					...
					<a class="btnList" href="${ctx}/house/ccmHousePetition/delete?id=${ccmHousePetition.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该重点上访人员吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHousePetition.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
				...
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHousePetition.id}&relevance_table=ccm_house_petition', '记录信息', '800px', '660px')" 
