		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmPlaceReligion">
				<tr>
					...
					<td><a href="${ctx}/religion/ccmPlaceReligion/form1?id=${ccmPlaceReligion.id}">${ccmPlaceReligion.placeName}</a></td>
					...
					<shiro:hasPermission name="religion:ccmPlaceReligion:edit">
						<td>
						<a class="btnList"
							onclick="parent.LayerDialog('${ctx}/event/ccmEventIncident/listCheck?placeId=${ccmPlaceReligion.basePlaceId}', '事件关联', '1100px', '700px')"
							title="事件关联"><i class="icon-random"></i></a> 
						<a class="btnList"
						    href="${ctx}/religion/ccmPlaceReligion/form1?id=${ccmPlaceReligion.id}"
