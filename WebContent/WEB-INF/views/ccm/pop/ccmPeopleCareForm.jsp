...
<body>
	<ul class="nav nav-tabs">
		<li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatCare">数据统计</a></li>
		<li><a style="width: 140px;text-align:center" href="${ctx}/pop/ccmPeople/listCareFirst">数据列表</a></li>
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/pop/ccmPeople/formCare?id=${ccmPeople.id}">特殊关怀<shiro:hasPermission name="pop:ccmPeople:edit">${not empty ccmPeople.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="pop:ccmPeople:edit">查看</shiro:lacksPermission></a></li>
		<c:if test="${not empty ccmPeople.id}">
<%-- 			<li><a href="${ctx}/log/ccmLogTail/formProCare?relevance_id=${ccmPeople.id}&relevance_table=ccm_people">跟踪信息<shiro:hasPermission name="log:ccmLogTail:edit">${not empty ccmLogTail.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="log:ccmLogTail:edit">查看</shiro:lacksPermission></a></li>
 --%>		</c:if>
	</ul>
	<form:form id="inputForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmPeople/saveCare" method="post" class="form-horizontal">
		<form:hidden path="id"/>
