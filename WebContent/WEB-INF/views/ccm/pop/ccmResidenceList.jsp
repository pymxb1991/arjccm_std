...
<div class="context" content="${ctx}"></div>
 <!-- 导入、导出模块 -->
	...
	<ul class="nav nav-tabs">
		<li class="active" style="width: 140px"><a class="nav-head" href="${ctx}/pop/ccmWork/">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmWork/list/10" method="post" class="breadcrumb form-search clearfix">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<c:forEach items="${page.list}" var="ccmPeople">
			<tr>
				...
				<td><a href="${ctx}/pop/ccmWork/form/residence?id=${ccmPeople.id}">
					${ccmPeople.name}</a>
				</td>
				<td>
				...
				<td><shiro:hasPermission name="pop:ccmPermanent:edit">
	    				<a class="btnList"  href="${ctx}/pop/ccmWork/form/residence?id=${ccmPeople.id}"  title="修改"><i class="icon-pencil"></i></a>
						<a class="btnList" href="${ctx}/pop/ccmWork/delete/residence?id=${ccmPeople.id}" onclick="return confirmx('确认要删除该实有人口吗？', this.href)"  title="删除"><i class="icon-remove-sign"></i></a>
				   <a class="btnList"
