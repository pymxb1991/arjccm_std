...
<script type="text/javascript">
	...
	function Pass(id) {
		...
		$.post('${ctx}/preview/ccmEventIncidentPreview/pass?status=02&id='+ id,function(data){ // 通过 02
			console.log(${ccmEventIncidentPreview.status ne '01'&&ccmEventIncidentPreview.status ne '02'});
			console.log(${ccmEventIncidentPreview.status});
			if(data.code == '200'){
	...
	function NoPass(id) {
		$.post('${ctx}/preview/ccmEventIncidentPreview/pass?status=03&id='+ id,function(data){// 拒绝 03
			console.log(${ccmEventIncidentPreview.status ne '01'&&ccmEventIncidentPreview.status ne '02'});
			console.log(${ccmEventIncidentPreview.status});
			if(data.code == '200'){
                setTimeout(function () {
...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li style="float: right;"><a class="btn btn-danger"
			href="${ctx}/preview/ccmEventIncidentPreview/1?status=03" style="color: #ffffff">已拒绝</a></li>
		<li style="float: right;"><a class="btn btn-success"
			href="${ctx}/preview/ccmEventIncidentPreview/1?status=02" style="color: #ffffff">已通过</a></li>
		<li style="float: right;"><a class="btn btn-warning btn-sm"
			href="${ctx}/preview/ccmEventIncidentPreview/1?status=01" style="color: #ffffff">未处理</a></li>
		<li style="float: right;"><a class="btn btn-primary btn-sm"
	...
	<form:form id="searchForm" modelAttribute="ccmEventIncidentPreview"
		action="${ctx}/preview/ccmEventIncidentPreview/1" method="post"
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>状态：</label> <form:select path="status"
					...
					<form:options items="${fns:getDictList('ccm_event_preview_status')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="first-line"><label>事件规模：</label> <form:select path="caseScope"
					class="input-medium">
					<form:option value="" label="全部" />
