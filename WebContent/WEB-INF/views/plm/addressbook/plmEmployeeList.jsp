...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/addressbook/plmEmployee/?types=1">通讯录列表</a></li>
		<shiro:hasPermission name="addressbook:plmEmployee:edit"><li><a href="${ctx}/addressbook/plmEmployee/form">通讯录添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="plmEmployee" action="${ctx}/addressbook/plmEmployee/list?types=1" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>职务：</label>
				<form:input path="duty" htmlEscape="false" maxlength="64" class="input-medium"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>备用联系电话</th>
				<th>备注</th>
				<shiro:hasPermission name="addressbook:plmEmployee:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="plmEmployee">
			<tr>
				<td><a href="${ctx}/addressbook/plmEmployee/form?id=${plmEmployee.id}">
					${plmEmployee.name}
				</a></td>
				<td>
				...
				<td>
					${fns:getDictLabel(plmEmployee.state,'plm_state','')}
				</td>
				<td>
					${plmEmployee.duty}
				</td>
		...
		<div class="row">
			<c:forEach items="${page.list}" var="plmEmployee">
				<div class="span4 spandiv">
					<div class="thumbnail">
					<a href="${ctx}/addressbook/plmEmployee/form?id=${plmEmployee.id}">
							<h4 title="${plmEmployee.name} ">姓名：${plmEmployee.name} </h4>
						</a>
						<div class="caption row-fluid">
							<div class=" spanimg" style="width: 30%">
									<img src="${plmEmployee.imgul}" onerror='this.src="${ctxStatic}/common/list/images/timg.jpg"' alt="无图片显示">
							</div>
						...
						<a class="btnList" href="${ctx}/addressbook/plmEmployee/form?id=${plmEmployee.id}" title="修改" ><i
									class="icon-pencil"></i></a>
						<a class="btnList" href="${ctx}/addressbook/plmEmployee/delete?id=${plmEmployee.id}" onclick="return confirmx('确认要删除该通讯录吗？', this.href)" title="删除"><i
