	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnCancel").on("click", function(){
				$('#flag').val('no');
				$("#inputForm").attr("action","${ctx}/car/apply/plmCarApplyScrap/apply");
				$("#inputForm").submit();
			});
			$("#btnSubmit").on("click", function(){
				$("#inputForm").attr("action","${ctx}/car/apply/plmCarApplyScrap/save");
				$("#inputForm").submit();
			});
...
<body>
	<form:form target="_parent" id="inputForm" style="margin: 30px 100px;" modelAttribute="plmCarApplyScrap" action="" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table id="table" class="table table-condensed">
			<tr>
				<td class="trtop" colspan="2">报废车辆<font color="red">*</font></td>
				<td colspan="6"><form:input type="hide" id="carIdsAll" path="car.id" htmlEscape="false" maxlength="64" class="input-xlarge required"/></td>
			</tr>
			<tr>
				<td class="trtop" colspan="2" style="width: 20%">品牌</td>
				<td colspan="2" style="width: 30%"><form:input id="carBrand" path="car.brand" htmlEscape="false" maxlength="64" class="input-xlarge "/></td>
				<td class="trtop" colspan="2" style="width: 20%">型号</td>
				<td colspan="2" style="width: 30%"><form:input id="carVmodel" path="car.vmodel" htmlEscape="false" maxlength="64" class="input-xlarge "/></td>
			</tr>
			<tr>
				<td class="trtop" colspan="2">报废类型<font color="red">*</font></td>
				<td colspan="6">
			...
			<tr>
				...
				<td colspan="6"><form:textarea path="reason" htmlEscape="false" rows="4" maxlength="1000" class="input-xxlarge "/></td>
			</tr>		
			<tr>
			...
			<c:if test="${not empty plmCarApplyScrap.procInsId}">
				<tr>
		...
		<div class="form-actions">
			...
			<c:if test="${ empty plmCarApplyScrap.procInsId}">
		...
		<c:if test="${not empty plmCarApplyScrap.id}">
			<a id="btnCancel" class="btn" href="javascript:;" onclick="history.go(-1)" ><i class="icon-reply"></i>返回</a>
			</c:if>
			<c:if test="${empty plmCarApplyScrap.id}">
