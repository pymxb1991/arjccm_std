					...
					$("#messageBox").text("输入有误，请先更正。");
						...
						$("#ascertain").on("click", function() {
							...
							for(var i=0;i<codeList.length;i++){
								...
								$.ajax({
										url : "${ctx}/equapply/plmEquApplyScrap/getEquipmentById",
										type : "post",
						...
						$("a[title='deleteDetail']").on(
								...
								function() {
									...
									$.ajax({
										url : "${ctx}/equapply/plmEquApplyScrap/deleteDetail?ids=1",
										type : "post",
										data : {
											"id" : id
										},
						...
						$("#btnSubmit").on("click",function() {
											...
											$('#flag').val('yes');
											$("#inputForm").attr("action","${ctx}/equapply/plmEquApplyScrap/save?type=2");
											$("#inputForm").submit();
											}
										});
						...
						$("#btnSubmit3").on("click",function() {
											...
											$("#inputForm").attr("action","${ctx}/equapply/plmEquApplyScrap/notCommit?type=2");
											$("#inputForm").submit();
											}
										});
						var idAndNum = "";
...
<body>
	...
	<form:form target="_parent" id="inputForm" modelAttribute="plmEquApply"
		action="${ctx}/equapply/plmEquApplyScrap/save?type=2" method="post"
		...
		<table id="table" class="table   table-condensed">
			...
			<tr>
				<td class="trtop" colspan="9">物资编号 <input type="text"
					...
					<a id="ascertain" 
					style="color: #fff; background-color: #19a7f0; width: 45px; height: 21px; background-repeat: no-repeat;"
					class="btn btn-inverse" href="javascript:;" ><i class="icon-ok"></i>确定</a>
					</td>
			</tr>
			<tr>
