...
public class ActTaskController extends BaseController {
	 ...
	 * @param endAct 结束活动节点名称
	 */
	@RequestMapping(value = "histoicTable")
	public String histoicTable(Act act, String startAct, String endAct, Model model,String colspan, String titleColspan){
		if (StringUtils.isNotBlank(act.getProcInsId())){
			List<Act> histoicFlowList = actTaskService.histoicFlowList(act.getProcInsId(), startAct, endAct);
			model.addAttribute("histoicFlowList", histoicFlowList);
			model.addAttribute("colspan", colspan);
			model.addAttribute("titleColspan", titleColspan);
		}
		return "modules/act/actTaskHistoricTable";
	}
	...
	public String form(Act act, HttpServletRequest request, Model model){
		...
		if (act.getProcInsId() != null){
			act.setHistoricProcIns(actTaskService.getHistoryProcIns(act.getProcInsId()));
		}
...
//		return "modules/act/actTaskForm";
	 ...
	 * 		vars.keys=flag,pass
	 * 		vars.values=1,true
	 * 		vars.types=S,B  @see com.arjjs.ccm.modules.act.utils.PropertyType
	 */
	...
	public String applyList(String category, HttpServletRequest request, HttpServletResponse response, Model model) {
	    Page<Object[]> page = new Page<Object[]>(request, response);
	    page = actTaskService.processList(page, category);
		model.addAttribute("page", page);
		model.addAttribute("category", category);
		return "plm/act/actTaskApplyList";
	}
