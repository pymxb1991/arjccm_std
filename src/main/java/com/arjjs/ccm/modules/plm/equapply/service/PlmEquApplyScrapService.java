...
public class PlmEquApplyScrapService extends CrudService<PlmEquApplyDao, PlmEquApply> {
	...
	public void notCommit(PlmEquApply plmEquApply) {
		...
		plmActService.save(plmEquApply.getPlmAct(), PlmTypes.EQU_APPLY_SCRAP, plmEquApply.getId());
	}
	@Transactional(readOnly = false)
	public void save(PlmEquApply plmEquApply) {
		...
		if (StringUtils.isBlank(plmEquApply.getProcInsId())) {
			...
			Map<String, String> returnMap = actUtConfigService.getProcInsId(PlmTypes.EQU_APPLY_SCRAP, plmEquApply,
					plmEquApply.getId());
