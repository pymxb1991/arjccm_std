 ...
 */
package com.arjjs.ccm.modules.pbs.sys.utils;
...
import com.arjjs.ccm.modules.sys.dao.MenuDao;
import com.arjjs.ccm.modules.pbs.sys.dao.PbsOfficeDao;
import com.arjjs.ccm.modules.sys.dao.RoleDao;
...
public class UserUtils {
	...
	private static PbsOfficeDao officeDao = SpringContextHolder.getBean(PbsOfficeDao.class);
//	private static PbsPartymemDao partymemDao = SpringContextHolder.getBean(PbsPartymemDao.class);
	private static PbsPartymembindDao pbsPartymembindDao = SpringContextHolder.getBean(PbsPartymembindDao.class);
	 ...
	 * @return
	 */
	public static PbsPartymem getPartymem() {
		Principal principal = getPrincipal();
		if (principal != null) {
			PbsPartymembind pbsPartymembind = new PbsPartymembind();
			pbsPartymembind.setSPrimarykey01(principal.getId());
	 ...
	 * @return
	 */
	public static List<PbsOffice> getOfficeList() {
		@SuppressWarnings("unchecked")
		List<PbsOffice> officeList = (List<PbsOffice>) getCache(CACHE_OFFICE_LIST);
		if (officeList == null) {
			User user = getUser();
			if (user.isAdmin()) {
				officeList = officeDao.findAllList(new PbsOffice());
			} else {
				PbsOffice office = new PbsOffice();
	 ...
	 * @return
	 */
	public static List<PbsOffice> getOfficeAllList() {
		@SuppressWarnings("unchecked")
		List<PbsOffice> officeList = (List<PbsOffice>) getCache(CACHE_OFFICE_ALL_LIST);
		if (officeList == null) {
			officeList = officeDao.findAllList(new PbsOffice());
		}
		return officeList;
